...
# Новая вспомогательная функция для подсчета строк в файле
def count_lines_in_file(filepath):
    """
    Подсчитывает количество строк в файле.
    Возвращает 0, если файл не найден или пуст, или если произошла ошибка.
    """
    ...
    try:
        ...
        print(f"Ошибка при подсчете строк в файле '{filepath}': {e}")
        return 0

def sort_chats_by_online_percentage(filepath, reverse_order=False):
    """
    Считывает файл с данными о чатах, сортирует строки по проценту онлайна
    и перезаписывает файл.
    Пример строки: https://t.me/mdps1164chat | 1,445 members | 13 online (0.90%)
    :param filepath: Путь к файлу для сортировки.
    :param reverse_order: Если True, сортирует от большего процента к меньшему. По умолчанию False (от меньшего к большему).
    """
    lines = []
    try:
        ...
        print(f"Ошибка: Файл '{filepath}' не найден для сортировки.")
        return False
    except Exception as e:
        print(f"Ошибка при чтении файла '{filepath}' для сортировки: {e}")
        return False
    ...
    # Сортируем список строк. Теперь с параметром reverse.
    # Если reverse_order=True, то сортировка будет убывающей (от большего к меньшему).
    # Если reverse_order=False (по умолчанию), то сортировка будет возрастающей (от меньшего к большему).
    sorted_lines = sorted(lines, key=get_percentage, reverse=reverse_order)

    try:
        with open(filepath, 'w', encoding='utf-8') as f:
            f.writelines(sorted_lines)
        print(f"Файл '{filepath}' успешно отсортирован по проценту онлайна (обратный порядок: {reverse_order}).")
        return True
    except Exception as e:
        print(f"Ошибка при записи отсортированных данных в файл '{filepath}': {e}")
        return False
...
def find_and_move_work_chats(source_folder, destination_folder, filename_to_find="Work_Chats_Statistics.txt", files_to_exclude=None):
    """
    Ищет и перемещает файлы filename_to_find из source_folder (и его подпапок)
    в destination_folder, исключая файлы из списка files_to_exclude.
    При перемещении, если файл с таким именем уже существует в папке назначения,
    добавляет временную метку к имени файла для обеспечения уникальности.
    Возвращает количество перемещенных файлов.
    """
    if files_to_exclude is None:
        files_to_exclude = []

    moved_count = 0
    print(f"Поиск и перемещение '{filename_to_find}' (исключая {files_to_exclude}) из '{source_folder}' и его подпапок в '{destination_folder}'...")
    try:
        ...
        for root, dirs, files in os.walk(source_folder):
            for file in files:
                # Проверяем, соответствует ли имя файла требуемому и не находится ли оно в списке исключений
                if file == filename_to_find and file not in files_to_exclude:
                    source_path = os.path.join(root, file)
                    destination_path_base = os.path.join(destination_folder, file)
                    
                    # Обработка дубликатов: добавляем временную метку, если файл с таким именем уже существует
                    destination_path = destination_path_base
                    counter = 1
                    while os.path.exists(destination_path):
                        base, ext = os.path.splitext(filename_to_find)
                        # Можно использовать int(time.time()) для уникальности, или просто счетчик
                        destination_path = os.path.join(destination_folder, f"{base}_{counter}{ext}")
                        counter += 1
                    
                    shutil.move(source_path, destination_path)
                    print(f"Файл '{file}' перемещен из '{root}' в '{destination_path}'.")
                    moved_count += 1
        return moved_count
    except Exception as e:
...
BAD_LINKS_FILE = "bad_links.txt" # Добавляем имя файла, который нужно исключить
...
COLLECT_TXT_FILE_PATTERN = r"сбор\d*(?:\(\d+\))?\.txt"
...
# *** ЛОГИРОВАНИЕ: Количество чатов после Шага 2 ***
print(f"\nЛОГ (Шаг 2): Количество ссылок в '{os.path.basename(private_chats_file)}': {count_lines_in_file(private_chats_file)}")
print(f"ЛОГ (Шаг 2): Количество ссылок в '{os.path.basename(public_chats_file)}': {count_lines_in_file(public_chats_file)}")
# *************************************************
...
destination_private_file = None # Инициализируем переменную здесь
try:
...
# Передаем список файлов для исключения. BAD_LINKS_FILE определен как "bad_links.txt"
total_moved_work_chats = find_and_move_work_chats(
    current_telegram_checker_folder, 
    UNPROCESSED_FOLDER_3, 
    filename_to_find=WORK_CHATS_STATISTICS_FILE, 
    files_to_exclude=[BAD_LINKS_FILE] 
)

# *** ЛОГИРОВАНИЕ: Количество чатов после Шага 6 ***
total_lines_in_work_chats = 0
if os.path.exists(UNPROCESSED_FOLDER_3):
    for filename in os.listdir(UNPROCESSED_FOLDER_3):
        # Убедимся, что мы считаем только Work_Chats_Statistics.txt (с возможными добавленными цифрами), а не bad_links.txt
        if WORK_CHATS_STATISTICS_FILE in filename and BAD_LINKS_FILE not in filename: 
            total_lines_in_work_chats += count_lines_in_file(os.path.join(UNPROCESSED_FOLDER_3, filename))
print(f"ЛОГ (Шаг 6): Общее количество ссылок в '{WORK_CHATS_STATISTICS_FILE}' (после перемещения в '{UNPROCESSED_FOLDER_3}'): {total_lines_in_work_chats}")
# *************************************************
...
# *** ЛОГИРОВАНИЕ: Количество чатов после Шага 7 (после фильтрации) ***
print(f"\nЛОГ (Шаг 7): Количество ссылок в '{os.path.basename(passed_file)}': {count_lines_in_file(passed_file)}")
print(f"ЛОГ (Шаг 7): Количество ссылок в '{os.path.basename(not_passed_file)}': {count_lines_in_file(not_passed_file)}")
print(f"ЛОГ (Шаг 7): Общее количество ссылок после фильтрации: {count_lines_in_file(passed_file) + count_lines_in_file(not_passed_file)}")
# *******************************************************************
...
try:
    if passed_file: # Проверяем, что файл 'прошли.txt' был найден
        # 8.1. Создание папки назначения 'ГОТОВЫЕ ЧАТЫ', если она не существует
        os.makedirs(READY_CHATS_FOLDER, exist_ok=True)
        # Формирование полного пути для копирования файла в 'ГОТОВЫЕ ЧАТЫ'
        destination_ready_chats_path = os.path.join(READY_CHATS_FOLDER, os.path.basename(passed_file))
        
        # 8.2. Обработка случая, если файл с таким именем уже существует в 'ГОТОВЫЕ ЧАТЫ'
        # Это предотвращает перезапись и сохраняет уникальность файлов.
        if os.path.exists(destination_ready_chats_path):
            base, ext = os.path.splitext(os.path.basename(passed_file))
            # Добавление временной метки к имени файла (например, "прошли_1678886400.txt")
            destination_ready_chats_path = os.path.join(READY_CHATS_FOLDER, f"{base}_{int(time.time())}{ext}")
            
        # 8.3. Копирование файла 'прошли.txt' в 'ГОТОВЫЕ ЧАТЫ'
        # `shutil.copy2` копирует файл и сохраняет метаданные (например, время создания/изменения).
        shutil.copy2(passed_file, destination_ready_chats_path)
        print(f"Файл '{os.path.basename(passed_file)}' скопирован в '{READY_CHATS_FOLDER}'.")
        
        # 8.4. Вызов новой функции для сортировки скопированного файла
        # Сортировка происходит только для файла в "ГОТОВЫЕ ЧАТЫ", не затрагивая оригинал.
        print(f"Запуск сортировки файла '{os.path.basename(destination_ready_chats_path)}' по проценту онлайна...")
        if sort_chats_by_online_percentage(destination_ready_chats_path):
            print(f"Файл '{os.path.basename(destination_ready_chats_path)}' в '{READY_CHATS_FOLDER}' успешно отсортирован.")
        else:
            print(f"Внимание: Не удалось отсортировать файл '{os.path.basename(destination_ready_chats_path)}'.")

        # 8.5. Перемещение оригинального файла 'прошли.txt' в 'НЕ отработанные' (Шаг 4)
        # Это позволяет сохранить оригинальный файл для последующих шагов обработки (Шаг 10: повторные ссылки тг.py)
        # и последующей очистки папки на Шаге 12.
        os.makedirs(UNPROCESSED_FOLDER_4, exist_ok=True) # Убеждаемся, что папка существует
        shutil.move(passed_file, os.path.join(UNPROCESSED_FOLDER_4, os.path.basename(passed_file)))
        print(f"Оригинальный файл '{os.path.basename(passed_file)}' перемещен в '{UNPROCESSED_FOLDER_4}'.")

    if not_passed_file: # Проверяем, что файл 'не_прошли*.txt' был найден
        # 8.6. Создание папки назначения 'НЕ' в 'ГОТОВЫЕ ЧАТЫ', если она не существует
        os.makedirs(READY_CHATS_NOT_FOLDER, exist_ok=True) 
        
        # Формируем путь для перемещения файла
        destination_not_passed_file_path = os.path.join(READY_CHATS_NOT_FOLDER, os.path.basename(not_passed_file))
        
        # Добавляем временную метку, если файл с таким именем уже существует в ГОТОВЫЕ ЧАТЫ\НЕ
        if os.path.exists(destination_not_passed_file_path):
            base, ext = os.path.splitext(os.path.basename(not_passed_file))
            destination_not_passed_file_path = os.path.join(READY_CHATS_NOT_FOLDER, f"{base}_{int(time.time())}{ext}")

        # Перемещение файла 'не_прошли*.txt' в эту папку.
        shutil.move(not_passed_file, destination_not_passed_file_path)
        print(f"Файл '{os.path.basename(not_passed_file)}' перемещен в '{READY_CHATS_NOT_FOLDER}'.")

        # *********** НОВОЕ: СОРТИРОВКА СКОПИРОВАННОГО ФАЙЛА НЕ_ПРОШЛИ ***********
        print(f"Запуск сортировки файла '{os.path.basename(destination_not_passed_file_path)}' по проценту онлайна (от большего к меньшему)...")
        if sort_chats_by_online_percentage(destination_not_passed_file_path, reverse_order=True):
            print(f"Файл '{os.path.basename(destination_not_passed_file_path)}' в '{READY_CHATS_NOT_FOLDER}' успешно отсортирован.")
        else:
            print(f"Внимание: Не удалось отсортировать файл '{os.path.basename(destination_not_passed_file_path)}'.")
        # ************************************************************
except Exception as e:
    print(f"Шаг 8: Общая ошибка при обработке файлов прошли/не_прошли: {e}")
    exit() # Завершаем скрипт, если произошла критическая ошибка
...
# 11. Поиск и перемещение прошли_без_дубликатов.txt и интеграция приватных чатов
print("\nШаг 11: Поиск и перемещение 'прошли_без_дубликатов.txt' и интеграция приватных чатов...")
time.sleep(5) 
...
try:
    # Определяем путь для конечного файла прошли_без_дубликатов.txt в UNPROCESSED_FOLDER_5
    final_no_duplicates_path = os.path.join(UNPROCESSED_FOLDER_5, os.path.basename(no_duplicates_file))
    os.makedirs(UNPROCESSED_FOLDER_5, exist_ok=True) 

    # *** ЛОГИРОВАНИЕ: Количество ссылок в прошли_без_дубликатов.txt до добавления приватных чатов ***
    print(f"\nЛОГ (Шаг 11 - до интеграции): Количество ссылок в '{os.path.basename(no_duplicates_file)}': {count_lines_in_file(no_duplicates_file)}")
    # ********************************************************************************************

    # Перемещаем файл прошли_без_дубликатов.txt в UNPROCESSED_FOLDER_5
    shutil.move(no_duplicates_file, final_no_duplicates_path)
    print(f"Файл '{os.path.basename(no_duplicates_file)}' перемещен в '{UNPROCESSED_FOLDER_5}'.")

    # Теперь обрабатываем файл приватных чатов
    if destination_private_file and os.path.exists(destination_private_file):
        print(f"Вырезание содержимого из '{os.path.basename(destination_private_file)}' и добавление в '{os.path.basename(final_no_duplicates_path)}'.")
        private_chat_content = []
        try:
            with open(destination_private_file, 'r', encoding='utf-8') as f_priv:
                private_chat_content = f_priv.readlines()

            with open(final_no_duplicates_path, 'a', encoding='utf-8') as f_dest: # 'a' для добавления
                f_dest.writelines(private_chat_content)
            print(f"Содержимое файла '{os.path.basename(destination_private_file)}' успешно добавлено в '{os.path.basename(final_no_duplicates_path)}'.")

            # Удаляем исходный файл приватных чатов, так как его содержимое "вырезано"
            os.remove(destination_private_file)
            print(f"Файл '{os.path.basename(destination_private_file)}' удален из '{READY_CHATS_FOLDER}', так как его содержимое было перемещено.")
        except Exception as e:
            print(f"Ошибка при обработке содержимого приватных чатов и файла без дубликатов: {e}")
            # Не завершаем скрипт здесь, так как основная цель шага (перемещение без дубликатов) выполнена
    else:
        print(f"Файл приватных чатов '{destination_private_file}' не найден или не был перемещен на Шаге 3. Пропускаем операцию вырезания/вставки.")

    # *** ЛОГИРОВАНИЕ: Количество ссылок в прошли_без_дубликатов.txt после добавления приватных чатов ***
    print(f"ЛОГ (Шаг 11 - после интеграции): Количество ссылок в '{os.path.basename(final_no_duplicates_path)}': {count_lines_in_file(final_no_duplicates_path)}")
    # *************************************************************************************************

except Exception as e:
    print(f"Шаг 11: Общая ошибка при обработке 'прошли_без_дубликатов.txt': {e}")
    exit()
...
print(f"\nОжидание файлов в '{PACKED_CHATS_FOLDER_5}' от скрипта 'Колич.чатов.py'...")
time.sleep(5) 
...
# *** ЛОГИРОВАНИЕ: Количество чатов после Шага 14 (из Чаты по пачкам в ГОТОВЫЕ ЧАТЫ) ***
total_lines_ready_chats_after_packed = 0
if os.path.exists(READY_CHATS_FOLDER):
    for filename in os.listdir(READY_CHATS_FOLDER):
        total_lines_ready_chats_after_packed += count_lines_in_file(os.path.join(READY_CHATS_FOLDER, filename))
print(f"ЛОГ (Шаг 14): Общее количество ссылок в '{READY_CHATS_FOLDER}' после перемещения из 'Чаты по пачкам': {total_lines_ready_chats_after_packed}")
# *************************************************************************************
...
if moved_final_chats == 0:
    ...
    try:
        ...
        print(f"Ошибка при попытке удалить пустую паку '{READY_CHATS_FOLDER}': {e}")
...
# *** ИЗМЕНЕНИЕ: Поиск файла "публичных чатов" для имени архивной папки ***
# В логах видно, что имя архивной папки берется из "приватных" чатов.
# Но так как "приватные" чаты перемещаются в "ГОТОВЫЕ ЧАТЫ", а затем их содержимое
# вырезается и добавляется в "прошли_без_дубликатов.txt", который потом перемещается
# в "НЕ отработанные" (5ChekLinksHUM), то "приватный" файл уже не будет в "ГОТОВЫЕ ЧАТЫ".
# Логичнее искать имя для архива из "публичных" файлов, если они попали в "ГОТОВЫЕ ЧАТЫ"
# через путь Work_Chats_Statistics.txt -> прошли.txt -> ГОТОВЫЕ ЧАТЫ.
# Или, что более надежно, взять имя из исходного public_chats_file, которое известно с Шага 2.

# Если нужно брать имя из файла, который *сейчас* в ГОТОВЫЕ ЧАТЫ, то
# нужно искать тот, который соответствует паттерну публичных или приватных чатов.
# Из вашего предыдущего лога: `Предупреждение: Не удалось найти файл '[а-яА-ЯёЁa-zA-Z]+_(\d+)_публичных(?:\s\(\d+\))?\.txt' в 'C:\Софт\ГОТОВЫЕ ЧАТЫ' для определения имени архивной папки. Будет использовано 'НЕИЗВЕСТНО'.`
# Это происходит потому, что `прошли_без_дубликатов.txt` уже содержит объединенные ссылки
# и не имеет в названии "публичных" или "приватных".

# Давайте вернемся к идее брать имя из public_chats_file, который был обнаружен на Шаге 2.
# Это более надежно, так как этот файл точно содержит нужное имя, и мы знаем его путь.
if public_chats_file:
    match = re.match(r"([а-яА-ЯёЁa-zA-Z]+)_", os.path.basename(public_chats_file))
    if match:
        archive_folder_name = match.group(1)
        print(f"Имя архивной папки взято из '{os.path.basename(public_chats_file)}': '{archive_folder_name}'.")
    else:
        print(f"Предупреждение: Не удалось извлечь имя из '{os.path.basename(public_chats_file)}'. Будет использовано '{archive_folder_name}'.")
else:
    print(f"Предупреждение: Файл публичных чатов не был найден на Шаге 2. Будет использовано '{archive_folder_name}'.")
...
# *** ЛОГИРОВАНИЕ: Количество чатов в ГОТОВЫЕ ЧАТЫ перед архивированием ***
total_lines_ready_chats_before_archive = 0
if os.path.exists(READY_CHATS_FOLDER):
    for filename in os.listdir(READY_CHATS_FOLDER):
        if os.path.isfile(os.path.join(READY_CHATS_FOLDER, filename)): # Убедимся, что это файл
            total_lines_ready_chats_before_archive += count_lines_in_file(os.path.join(READY_CHATS_FOLDER, filename))
print(f"ЛОГ (Шаг 16 - перед архивированием): Общее количество ссылок в '{READY_CHATS_FOLDER}': {total_lines_ready_chats_before_archive}")
# ********************************************************************
